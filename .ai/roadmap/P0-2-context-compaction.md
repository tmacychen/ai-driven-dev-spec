...
实现**任务级上下文压缩**，确保当前 Workspace 的上下文空间始终最优，同时通过链式结构实现可回溯的无限上下文。
...
│
├── dev-001_20260409-153000.ses  # Dev Agent 的 Workspace 文件
├── dev-001_20260409-153000-ses1.log  # 工具输出 log
├── dev-001_20260409-153000-ses2.log  # 同一 Workspace 的第 2 个 log
├── dev-001_20260409-153000.mem  # 记忆文件（高密度 Markdown 归档）
│
├── pm-001_20260409-154000.ses   # PM Agent 的 Workspace 文件
├── pm-001_20260409-154000.mem
│
└── reviewer-001_20260409-160000.ses  # Reviewer Agent 的 Workspace 文件

命名格式: {role}-{sequence}_{timestamp}.{ext}
```

**向后兼容**：旧格式 `{timestamp}.{ext}` 仍然支持，视为无角色标记的 Workspace。
