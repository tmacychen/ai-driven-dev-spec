...
## Overall Status
- ✅ Completed: 16 (P0+P1+P2 功能 + TUI核心统一)
- 🔄 In Progress: 0
- ⏳ Pending: 0
- ⚠️ Blocked: 0
- 🔴 Regression: 0
...
## Next Step
CLI 和 TUI 已完全共享 AgentCore 核心逻辑。待集成测试验证后合并到 main。
...
5. **CLI agent_loop.py 迁移到 AgentCore**（统一完成）
   - AgentLoop 变为 AgentCore 的 CLI 适配层
   - CLICallbacks 实现 AgentCallbacks 接口
   - 命令处理（/help, /quit, /model 等）保留在 CLI 层
   - Rich 渲染 + prompt_toolkit 输入保留在 CLI 层
   - 修正 AgentCore API 对齐（budget/compactor/memory 方法名）
